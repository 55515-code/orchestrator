# References

Core tools used in this substrate:

- uv: https://docs.astral.sh/uv/
- mise: https://mise.jdx.dev/
- direnv: https://direnv.net/
- LangChain: https://python.langchain.com/
- LangGraph: https://langchain-ai.github.io/langgraph/
- Hugging Face smolagents: https://huggingface.co/docs/smolagents
- Hugging Face agent-collabs: https://github.com/huggingface/agent-collabs
- Hugging Face Transformers: https://huggingface.co/docs/transformers
- Hugging Face Hub: https://huggingface.co/docs/huggingface_hub
- vLLM: https://docs.vllm.ai/
- SGLang: https://docs.sglang.ai/
- Hugging Face Optimum: https://huggingface.co/docs/optimum
- MkDocs Material: https://squidfunk.github.io/mkdocs-material/
- FastAPI: https://fastapi.tiangolo.com/
- Caddy: https://caddyserver.com/docs/
- cloudflared: https://developers.cloudflare.com/cloudflare-one/connections/connect-networks/
- Tailscale: https://tailscale.com/kb/
- GitHub OAuth scopes: https://docs.github.com/en/apps/oauth-apps/building-oauth-apps/scopes-for-oauth-apps
- GitLab personal access tokens: https://docs.gitlab.com/user/profile/personal_access_tokens/
- Slack OAuth v2: https://docs.slack.dev/authentication/installing-with-oauth/
- Notion authorization: https://developers.notion.com/guides/get-started/authorization
- chezmoi docs: https://www.chezmoi.io/
- yadm docs: https://yadm.io/docs/getting_started
- GNU Stow manual: https://www.gnu.org/software/stow/manual/stow.html
- Syncthing docs: https://docs.syncthing.net/
- restic docs: https://restic.readthedocs.io/en/stable/
- rclone docs: https://rclone.org/docs/
- Proton Mail IMAP/SMTP and Bridge setup: https://proton.me/support/imap-smtp-and-pop3-setup
- Proton Mail Bridge startup guide: https://proton.me/support/automatically-start-bridge
- Proton Drive SDK preview: https://proton.me/blog/proton-drive-sdk-preview
- Proton Drive for Business support: https://proton.me/support/drive/proton-drive-for-business
- Proton VPN for Business support: https://proton.me/support/business/vpn
- Proton VPN business setup guide: https://proton.me/support/setup-vpn-business
- Proton VPN business product page: https://proton.me/business/vpn

- Caching and memoization:
  - diskcache: https://diskcache.readthedocs.io/
  - cachetools: https://cachetools.readthedocs.io/
  - Joblib Memory: https://joblib.readthedocs.io/

Security/device and Android standards used by this project:

- Hak5 USB Rubber Ducky docs: https://docs.hak5.org/hak5-usb-rubber-ducky/
- Hak5 payload repository: https://github.com/hak5/usbrubberducky-payloads
- Flipper developer docs (BadUSB/Sub-GHz formats): https://developer.flipper.net/flipperzero/
- Kali metapackages: https://www.kali.org/docs/general-use/metapackages/
- Kali removed tools lifecycle: https://www.kali.org/docs/tools/removed-tools/
- Parrot mirror/signature docs: https://www.parrotsec.org/docs/mirrors/mirrors-list/
- MITRE ATT&CK resources: https://attack.mitre.org/resources/
- Atomic Red Team: https://github.com/redcanaryco/atomic-red-team
- Sigma: https://sigmahq.io/
- Nuclei templates docs: https://docs.projectdiscovery.io/templates/introduction
- Android platform-tools: https://developer.android.com/tools/releases/platform-tools
- Apktool: https://apktool.org/
- JADX: https://github.com/skylot/jadx
- MobSF: https://github.com/MobSF/Mobile-Security-Framework-MobSF
- Frida CLI docs: https://frida.re/docs/frida-cli/
- OWASP MASVS/MASTG: https://mas.owasp.org/
