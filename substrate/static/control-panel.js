...
class ControlPanel {
    ...
    async loadAutomations() {
        ...
        grid.innerHTML = actions.map(a => `
            <button class="automation-card glass" data-action="${a.name}" ${a.takes_input ? 'data-input="1"' : ''}>
                <div class="automation-card__name">${a.name.replace(/_/g, ' ')}</div>
                <div class="automation-card__desc">${a.description || ''}</div>
            </button>
        `).join('');
            ...
            <table>
                ...
                <tbody>
                    ${runs.slice(0, 20).map(run => `
                        <tr>
                            <td><code>${run.run_id?.substring(0, 8) || 'N/A'}</code></td>
                            <td>${run.task_id || 'N/A'}</td>
                            <td>${run.repo_slug || 'N/A'}</td>
                            <td>${run.stage || 'local'}</td>
        ...
        list.innerHTML = recentRuns.map(run => `
            <div class="activity-item">
                ...
                <div class="activity-content">
                    <div class="activity-title">Task ${run.status}: ${run.task_id || 'unknown'}</div>
                    <div class="activity-meta">${run.repo_slug || 'unknown'} • ${this.timeAgo(run.started_at)}</div>
                </div>
            </div>
        `).join('');
    }
    ...
    async saveConfig() {
        const config = {
            ...
            access_token: document.getElementById('whatsappAccessToken').value,
            app_secret: document.getElementById('whatsappAppSecret').value,
            verify_token: document.getElementById('whatsappVerifyToken').value,
            webhook_url: document.getElementById('whatsappWebhookUrl').value
        };

        // Validate required fields
        if (!config.phone_number_id || !config.access_token || !config.app_secret || !config.verify_token) {
    ...
    async generateQR() {
        try {
            ...
            qrContainer.innerHTML = `<img src="${response.qr_code}" alt="WhatsApp QR Code">`;
    ...
    async loadWhatsAppConfig() {
        try {
            ...
            if (config.verify_token) {
                document.getElementById('whatsappVerifyToken').value = config.verify_token;
            }
